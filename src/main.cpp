...
void loop() {
  ...
  // заполнение массива временем
  unsigned long currentMillis = millis();
  if(currentMillis - previousMillisMin > secondinterval)
  {
   second=second+1;
   second==60?second=0:second=second;
   previousMillisMin = currentMillis;
   showtime(hour,minute,second);
  }
  // отображение общего массива в экран
  currentMillis = millis();
  if(currentMillis - previousMillisScr > secondinterval)
  {
   previousMillisScr = currentMillis;
   writearrtoscr();// собственно отображаем экран  
  }
 }